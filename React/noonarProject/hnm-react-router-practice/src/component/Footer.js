            ...
            <ul>
                <li>
                    <dl className='site-icon'>
                        {
                            siteDate.map( item => <dd key={item.id}><a href={item.href} target='_blank'><img width={20} src={`assets/${item.src}`} alt="" /></a></dd> )
                        }
                    </dl>
                </li>
                <li>
                ...
                <li><p>사업자 정보 확인</p></li>
                <li><p>지급보증안내</p></li>
                <li><p><img width={40} className='logo' src={'assets/H&M-Logo.png'} onClick={goToHome} alt="" /></p></li>
                <li><p>대한민국 | ₩</p></li>
            </ul>
        </div>
    );
